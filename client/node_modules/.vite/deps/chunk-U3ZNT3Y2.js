...
function useListChangeNotifiers() {
  ...
  const notifySelectionChanged = React2.useCallback((newSelectedItems) => {
    messageBus.publish(SELECTION_CHANGE_TOPIC, newSelectedItems);
  }, [messageBus]);
  const notifyHighlightChanged = React2.useCallback((newHighlightedItem) => {
    messageBus.publish(HIGHLIGHT_CHANGE_TOPIC, newHighlightedItem);
  }, [messageBus]);
  const registerSelectionChangeHandler = React2.useCallback((handler) => {
    return messageBus.subscribe(SELECTION_CHANGE_TOPIC, handler);
  }, [messageBus]);
  const registerHighlightChangeHandler = React2.useCallback((handler) => {
...
function useForcedRerendering() {
  const [, setState] = React7.useState({});
...
function useListItem(parameters) {
  ...
  const listContext = React8.useContext(ListContext);
  if (!listContext) {
  ...
  const createHandleClick = React8.useCallback((other) => (event) => {
    var _other$onClick;
  ...
  const createHandlePointerOver = React8.useCallback((other) => (event) => {
...
var CompoundComponentContext = React9.createContext(null);
...
function useCompoundParent() {
  const [subitems, setSubitems] = React9.useState(/* @__PURE__ */ new Map());
  const subitemKeys = React9.useRef(/* @__PURE__ */ new Set());
  const deregisterItem = React9.useCallback(function deregisterItem2(id) {
  ...
  const registerItem = React9.useCallback(function registerItem2(id, item) {
  ...
  const sortedSubitems = React9.useMemo(() => sortSubitems(subitems), [subitems]);
  const getItemIndex = React9.useCallback(function getItemIndex2(id) {
...
//# sourceMappingURL=chunk-U3ZNT3Y2.js.map
